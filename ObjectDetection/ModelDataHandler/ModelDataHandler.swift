...
*/
  ...
  func formatResults(boundingBox: [Float], outputClasses: [Float], outputScores: [Float], outputCount: Int, width: CGFloat, height: CGFloat) -> [Inference]{
    ...
    print("inside formatResults 1 outputCount *****", outputCount)
    if (outputCount == 0) {
      return resultsArray
    }
    print("inside formatResults boundingBox *****", boundingBox.description)
    print("inside formatResults outputClasses *****", outputClasses.description)
    print("inside formatResults outputScores *****", outputScores.description)
    print("inside formatResults outputCount *****", outputCount.description)
    ...
    guard score >= threshold else {
      ...
      // Translates the detected bounding box to CGRect.
        ...
        */
    /*
      rect.origin.y = max(1, CGFloat(boundingBox[4*i]) * height)
      rect.origin.x = max(1, CGFloat(boundingBox[4*i+1]) * width)
      rect.size.height = min((CGFloat(boundingBox[4*i+2]) - rect.origin.y), height)
      rect.size.width = min((CGFloat(boundingBox[4*i+3]) - rect.origin.x), width)
     */
  ...
  private func loadLabels(fileInfo: FileInfo) {
    ...
    //i can see the labels, it's correct
    //print("LABELS *****", labels.description)
  }
